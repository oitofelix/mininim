...
this by invoking the command:

```
sudo apt-get install '^liballegro.*5-dev$'
```

When building the sources, the usual incantation `./configure && make`
is often enough.  The installation step (`make install`) is optional
as MININIM is able to run directly from its source directory (provided
you don't make a VPATH build, in which case you need to specify the
data path.  See the `--data-path` option).
